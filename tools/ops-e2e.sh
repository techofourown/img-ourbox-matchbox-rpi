...
newest_img_xz() {
  ...
  local base
  base="$(basename "${img}")"
  if [[ "${base}" == installer-* || "${base}" == *installer* ]]; then
    die "refusing non-OS artifact selected as OS image: ${img}"
  fi
  [[ "${base}" == img-ourbox-matchbox-${OURBOX_TARGET,,}-*.img.xz ]] || \
    die "selected image does not match expected OS glob (img-ourbox-matchbox-${OURBOX_TARGET,,}-*.img.xz): ${img}"
